...
After the first user registers, promote them to admin:

[source,bash]
----
# SQLite
sqlite3 warmdesk.db "UPDATE users SET global_role='admin' WHERE id=1;"

# PostgreSQL
psql -U warmdesk -c "UPDATE users SET global_role='admin' WHERE id=1;"
----

Restart the server (not strictly required, but good practice).
Future admin accounts can be promoted from the admin panel.
