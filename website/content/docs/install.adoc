...
https://github.com/tonk/warmdesk/releases[releases page].
...
----
# Linux x86-64
wget https://github.com/tonk/warmdesk/releases/latest/download/warmdesk-{warmdesk-version}-linux-amd64.tar.gz
mkdir warmdesk && tar -xzf warmdesk-{warmdesk-version}-linux-amd64.tar.gz -C warmdesk
cd warmdesk

# Linux ARM64
wget https://github.com/tonk/warmdesk/releases/latest/download/warmdesk-{warmdesk-version}-linux-arm64.tar.gz
...
A dedicated `<Location "/api/v1/ws">` block with `ProxyPass ws://…` and `ProxyTimeout 86400` handles WebSocket connections.
It must appear *before* the catch-all `ProxyPass /` so Apache routes WebSocket traffic correctly.

== Desktop apps

Pre-built Tauri 2 desktop apps are available on the
https://github.com/tonk/warmdesk/releases[releases page]:
