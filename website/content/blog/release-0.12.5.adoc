...
WarmDesk `{warmdesk-version}` adds a Contract Rates Overview so you can see every standby and after-hours rate across all accessible customers in one place, along with a corrected database schema reference.
...
The link:../database-schema[database schema reference] had several stale entries that have now been corrected:
...
The link:../api[REST API reference] gained a *Customers and contracts* section listing all 10 endpoints.
