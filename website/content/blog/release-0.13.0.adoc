...
WarmDesk `{warmdesk-version}` closes a small but real gap: passkeys were only ever visible from inside the Security settings tab, with no quick way to confirm they were registered — and no way at all for an admin to check or manage another user's passkeys.
