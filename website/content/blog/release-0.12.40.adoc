...
WarmDesk `{warmdesk-version}` rounds out the time-tracking report with a custom date range and per-row billable detail, and gives the desktop app a proper system tray icon.

== Report on any date range

The Report tab's period selector was limited to Week, Month, or Year.
`{warmdesk-version}` adds *Custom range*: pick it and a popup opens with *Start date* / *End date* pickers that follow your date format setting, just like every other date field in WarmDesk.
