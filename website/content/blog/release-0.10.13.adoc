...
Download `{warmdesk-version}` from the link:download.adoc[download page] or upgrade your existing installation — database schema changes are applied automatically on startup (new `end_day_offset` column on contract time slots).
