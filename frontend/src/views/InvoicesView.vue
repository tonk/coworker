        ...
        <table class="invoice-table">
          ...
          <tbody>
            <tr v-for="inv in invoices" :key="inv.id">
              ...
              <td><span :class="['status-badge', `status-${inv.status}`]">{{ $t(`invoice.status_${inv.status}`) }}</span></td>
              <td class="col-right col-total">{{ inv.currency }} {{ inv.total.toFixed(2) }}</td>
              <td class="col-date">
                <span v-if="inv.due_date">{{ formatDate(inv.due_date) }}</span>
...
watch([filterCustomer, filterStatus], load)
onMounted(load)
</script>
