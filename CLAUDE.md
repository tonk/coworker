...
| `TicketMessage` | `TicketID`, `UserID`, `Body` — internal messages on a ticket |
...
- **`TicketDetailView.vue`** — full ticket detail with inline title editing, status/priority/type/assignee dropdowns, tag management, SLA card, linked tickets/cards panel, internal messages with attachments, and the pending reminder `DatePicker`.
