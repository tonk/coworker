...
.Board view — drag-and-drop, optional WIP limits
image::../screenshots/03-board.png[Board view — drag-and-drop, optional WIP limits, 900]
...
The server refuses to start if `jwt_secret` is still set to its default value, or if `allowed_origins` contains `*` in `release` mode.

API keys use `X-API-Key: <key>` or `?api_key=<key>` and are intended for CI/CD pipelines and automation scripts.
Uploaded file MIME types are detected server-side from the file content — the client-supplied `Content-Type` header is ignored.
...
port: 8080
jwt_secret: change-me-in-production   # server refuses to start at this default — change it
EOF
...
Open `http://localhost:8080` and register the first user—that account becomes **admin** automatically on an empty database.
